package com.example.blogmanagementsystem.web;

import com.example.blogmanagementsystem.domain.Article;
import com.example.blogmanagementsystem.domain.User;
import com.example.blogmanagementsystem.service.ArticleService;
...
public class ArticleController {
    ...
    public String getPost(@AuthenticationPrincipal UserDetails userDetails, @PathVariable String id, Model model) throws Exception {
    ...
    @GetMapping("/edit/{id}")
    public String editPost(@AuthenticationPrincipal UserDetails userDetails, @PathVariable String id, Model model) throws Exception {
        Optional<Article> article = articleService.getById(id);
        if (article.isPresent()) {
            model.addAttribute("article", article.get());
        } else {
    ...
    private String throwNotFoundException(String id) throws Exception {
        throw new Exception("Article Not Found for " + id);
    }
    ...
    @GetMapping("/rest")
    @ResponseBody
    public Page<Article> articleRest(@RequestParam(required = false, value = "page") Integer page,
                                     @RequestParam(required = false, value = "size") Integer size) {
